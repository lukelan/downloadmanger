...
{
    if([size floatValue]>=1024*1024)//Greater than 1M, then converted into a string of M units
    {
        return [NSString stringWithFormat:@"%1.2fM",[size floatValue]/1024/1024];
    }
    else if([size floatValue]>=1024&&[size floatValue]<1024*1024) //Less than 1M, but more than 1KB, then converted into KB units
    {
        return [NSString stringWithFormat:@"%1.2fK",[size floatValue]/1024];
    }
    else//The rest are less than 1K, then converted into B units
    {
...
//        {
//            FPTFileDownloadModel *finishedFile=[[FPTFileDownloadModel alloc] init];
//            finishedFile.fileName=fileName;
//            finishedFile.targetPath = [pathstr stringByAppendingPathComponent:fileName];
//            //Get the file size based on file name
...
+(NSString *)getDiskSpaceInfo{
    ...
    if (dictionary)
    {
        ...
        NSString *infostr = [NSString stringWithFormat:@"%.2f GB Available / Total %.2f GB", ((totalFreeSpace/1024.0f)/1024.0f)/1024.0f, ((totalSpace/1024.0f)/1024.0f)/1024.0f];
        return infostr;
